...
appears as a clause in the sentence about how the change was produced.
...
| ci_failing | A check run on the head commit failed, timed out, or was cancelled | Fix CI. |
...
| model_commit_email | the commits are authored from a model service address | A commit author email matches `model_email_patterns` |
...
The sentence about the author's standing, with pull requests here,
merged elsewhere, account age, public repositories, and followers, is
composed from the profile and is not a signal.
